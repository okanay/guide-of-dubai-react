      ...
      <div className="mx-auto max-w-main pt-10">
        ...
        <header className="mb-4 flex items-start justify-between sm:items-end">
          <div className="flex flex-col gap-y-1">
            <h2 className="text-size-2xl font-bold">{t('lux-activities.title')}</h2>
            <p className="text-size-base text-gray-700"> {t('lux-activities.description')}</p>
          </div>
          <Link
            to="/$lang/not-found"
        ...
        <div className="relative">
          ...
          >
            {MOCK_POPULAR_ACTIVITIES.map((activity, index) => (
              <div
                key={activity.id}
                ref={setCardRef(index)}
                className="w-[calc(25%_-_1rem)] min-w-[260px] shrink-0 snap-start md:w-[calc(100%_/_3_-_1rem)] md:min-w-[360px]"
              >
                <ActivityCardLux activity={activity} />
              </div>
            ))}
          </div>
        </div>
